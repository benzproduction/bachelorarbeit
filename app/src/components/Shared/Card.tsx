...
const CardPattern = React.forwardRef<HTMLElement, CardProps>((props: CardProps, ref) => {
    ...
    return (
        <div className={classNames(styles.ap_pattern_card, className)} role="link" tabIndex={0}
        onClick={(event) => onClick?.(event, item?.link, item?.params)}
        onKeyDown={(event) => onKeydown?.(event, item?.link, item?.params)}>
            <div className={classNames(styles.ap_pattern_card_according , styles.component_thumb)}>
            ...
            <span className={styles.ap_pattern_card_name}>
                {item?.name}
            </span>
            </div>
            <p className={styles.ap_pattern_card_desc}>
                {item?.description}
            </p>
            </div>
